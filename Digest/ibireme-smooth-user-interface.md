...
双缓冲 (double buffer) 是一个 buffer 由 GPU 写入，另一个供显示器读取，到了时间再对换。详细的过程是：当显示器绘制完 A-buffer 中的像素之后（即绘制完第 n + 1 帧），就会去读取 B-buffer（ GPU 已经写入好的第 n + 2 帧），而 GPU 则转到这个被显示器已经读完的 A-buffer ，向其中写入第 n + 3 帧的像素信息。
